...
export interface ViewColumnConfig {
  ...
  // V2 Online Ledger Column Configurations
  sourceType?: 'database' | 'current_workflow' | 'workflow_history' | 'calculated' | 'manual';
  databaseField?: string;       // References the system/database field path, e.g. 'code', 'name', 'ship', etc.
  historyNodeAttr?: string;     // References the workflow node attribute, e.g. '寄出', '结算'
  historyRule?: 'enter' | 'leave'; // 'enter' = 进入节点的时间, 'leave' = 离开节点的时间 (Default: leave)
  calculatedFormula?: 'unpaid_amount' | 'progress_rate' | 'days_elapsed';
  manualFieldType?: 'text' | 'number';
}
...
export interface DataCenterConfig {
  ...
  // V2 Online Ledger configuration extensions
  granularity?: 'contract' | 'batch' | 'auto'; // Default: 'auto'
  manualData?: Record<string, Record<string, string>>; // Maps rowId -> colField -> cell value
}
